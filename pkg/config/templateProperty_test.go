...
func Test_getValidationRule(t *testing.T) {
	tests := []struct {
		...
		{"valid map with regex keys", "regex", map[string]any{"[a-z]+": "description", "\\d+": "another"}, true},
		{"invalid map with bad regex key", "regex", map[string]any{"[a-z": "description"}, false},
		{"valid map with regex values", "mapregexvalues", map[string]any{"description": "[a-z]+", "another": "\\d+"}, true},
		{"invalid map with bad regex value", "mapregexvalues", map[string]any{"description": "[a-z"}, false},
