...
func Test_getValidationRule(t *testing.T) {
	tests := []struct {
		...
		{"valid map with regex values", "regex", map[string]any{"description": "[a-z]+", "another": "\\d+"}, true},
		{"invalid map with bad regex value", "regex", map[string]any{"description": "[a-z"}, false},
